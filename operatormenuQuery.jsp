...
<body>
	...
	<form action="menuQuery.jsp" method="get">
		查询菜单：<input type="text" name="keyword"> <input type="submit"
			value="提交">
	</form>
	<br>
	<p>
		Keyword: &nbsp;<%=request.getParameter("keyword")%></p>
		...
		<%
			...
			Class.forName("oracle.jdbc.driver.OracleDriver"); // 加载数据库驱动s
			...
			if (request.getParameter("keyword") == null)
				...
				rs = stmt.executeQuery(
						"SELECT * FROM food where foodname LIKE '%"+ request.getParameter("keyword") + "%' or foodkind LIKE '%"+ request.getParameter("keyword")+"'" );
			int count=0;
			while (rs.next()) {
		%>
		<div id="food1" class="horizontal" style="padding: 20px">
					<table class="pure-table">
						...
						<tbody>
							<tr>
								<td background="cake7.png" height="285px" width="300px"></td>
								<%count++; %>
							</tr>
							<tr>
					...
					<tr>
						<td id="foodnum"><%="Stock： "+rs.getString("foodnum")%></td>
							...
							<tr>
								<td>
									<form method="get" id="deleteForm" action="DeleteFood">
										<input type="hidden" name="foodName"
											value="<%=rs.getString("foodname")%>">
										<button class="button1" type="submit">删除</button>
									</form>
